...
MarkFile = require './mark-files'
PaneView = require './side-pane'
...
class TreeView extends View
  ...
  annotateDirectory: (fileName) ->
    ...
    try
      ...
      console.log(annotatorFile)
      filestoBeMarked = @getAnnotationsinFile(annotatorFile)
      console.log("#{fileName.getEntries()}")
      for elem in testlist
        isFile = elem instanceof FileView
        console.log("Is instance of file? : " + isFile)
        console.log("Element name: " + elem.getName())
    ...
    for subdir in testlist
      isDir = subdir instanceof DirectoryView
      if(isDir)
        hasAnnotations = hasAnnotations || @annotateDirectory(subdir)
    if(hasAnnotations)
      console.log("Inside has Annotations......")
      fileName.getDirectoryName().classList.add('isAnnotated')
    return hasAnnotations

  markAnnotatedFile: ->
    return unless entry = @selectedEntry()
    isDirectory = entry instanceof DirectoryView
    if(isDirectory)
      @annotateDirectory(entry)
